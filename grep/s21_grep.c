...
void mf();
void s21_vyb(regex_t *sk, int len, int argc, char **argv, int curr_pos);
regex_t *assoc(regex_t *reg1, regex_t *reg2, int e_n, int f_n);
regex_t *assoc_s(int argc, char *argv[], int *e_n, int *err);
regex_t *parsing_f(int argc, char *argv[], int *f_n, int *err);
regex_t *assoc_4(int *curr_pos, int argc, char *argv[], regex_t *sk, int *len);
regex_t *fl_f(char **file_names, int len, int *f_n, int *error);
void pasrsing(int n, char *argv[], int *err);
char *check(char *str);
...
int main(int argc, char **argv) {
    ...
    int e_n = 0, f_n = 0;
    int pos = 1;
    pasrsing(argc, argv, &err);
    regex_t *sk = assoc_s(argc, argv, &e_n, &err);
    regex_t *regex_from_file = parsing_f(argc, argv, &f_n, &err);
    int len = e_n + f_n;
    sk = assoc(sk, regex_from_file, e_n, f_n);
    if (!e_n && !f_n) {
        sk = assoc_4(&pos, argc, argv, sk, &len);
    }
    if (!err) {
        s21_vyb(sk, len, argc, argv, pos);
    }
    if (sk != NULL) {
        free(sk);
    }
        ...
        if (f != NULL) {
            ...
            while (on) {
                ...
                if ((on && c == '\0') || (!on && *line != '\0')) {
                    ...
                    matched = 0;
                    for (int k = 0; k < len; k++) {
                        int ret = regexec(sk + k, line, nmatch, pmatch, 0);
                        if (!ret) {
                            matched = 1;
                            break;
                        }
                        ...
                        if (!grep.c) {
                            ...
                            if (grep.o && !grep.v) {
                                ...
                                printf("%.*s\n", (int)(pmatch[0].rm_eo - pmatch[0].rm_so),
                                    &line[pmatch[0].rm_so]);
                                for (int k = 0; k < len; k++) {
                                    while (!regexec(sk + k, line + appender,
...
regex_t *assoc_s(int argc, char *argv[], int *e_n, int *err) {
    ...
    while (i < argc && !(*err)) {
        if (!strcmp(argv[i], "-e")) {
            ...
            regex_t *tmp = realloc(sk, count * sizeof(regex_t));
            if (tmp != NULL) {
                sk = tmp;
            }
            ...
            tmp_str = check(tmp_str);
            if (grep.i) {
                regcomp(sk + (count - 1), tmp_str, REG_ICASE);
            } else {
                regcomp(sk + (count - 1), tmp_str, 0);
            }
            free(tmp_str);
        }
        i++;
...
regex_t *parsing_f(int argc, char *argv[], int *f_n, int *err) {
    ...
    int file_len = count;
    regex_t *ret = fl_f(file, file_len, f_n, err);
...
regex_t *fl_f(char **file, int len, int *f_n, int *error) {
    regex_t *sk = NULL;
            ...
            *error = 1;
            ...
            while (on) {
                ...
                if (c == '\0') {
                    ...
                    regex_t *tmp = realloc(sk, (*f_n) * sizeof(regex_t));
                    if (tmp != NULL) {
                        sk = tmp;
                    }
                    if (grep.i) {
                        regcomp(sk + *f_n - 1, line, REG_ICASE);
                    } else {
                        regcomp(sk + (*f_n) - 1, line, 0);
                    }
                    free(line);
                    line_len = 0;
                    line = malloc(0);
